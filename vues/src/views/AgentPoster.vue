...
<template>
	<div class='agent-poster'>
		...
		<div class='add-poster' :style="{backgroundColor:this.$store.state.color}">
			...
			<div class='poster-title'>上传招商正文海报</div>
			<div class='poster-subtitle'>尺寸：宽度680像素(不超过10张图)</div>
		</div>
		<div class='add-remind'>将作为招商海报供代理了解推广事项</div>
		...
		<div class='complete'>
			<div class='complete-t' @click='changePreagentPoster'>上传完成并提交使用</div>
		</div>
	</div>
</template>
		...
		methods:{
			getData(){
				...
				.then(function(res){
					...
					that.agentPoster = res.data.poster;
				})
			},
			upFile(){
				if(this.agentPoster.length===10){
