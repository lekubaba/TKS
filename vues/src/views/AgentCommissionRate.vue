		...
		<div class='container'>
			<div class='container-a'>
				...
				<transition name="fade">
					<div v-if="show1">
						<div class='remind'>
							城市代理直推的首笔借款金额比例，请填写纯小数，比如1.5个点就填写0.015，不要填1.5%。
						</div>
						<div class='contentx'>
							<input class='inputx' v-model.number="ratea" placeholder="代理直推比例" type="number" ref='extension' min="0" max="1.00" step="0.01" oninput="value=value.replace(/[^\d.]/g,'')" onkeyup="this.value=this.value.replace(/[, ]/g,'')">
							<button class='confirmx' @click='toSaveExtension'>确认分配</button>
						</div>
						<div class='remind' v-if="ratea!=='城市代理直推比例'"> 您填写了：<span style='color:#FF654D'>{{ratea}}</span> , 如果城市代理直推放款10万。他自己能获得的佣金是：<span style='color:#FF654D'>{{rateA}}</span>  元。</div>
					</div>
				</transition>
			</div>
		</div>
		<div class='container1'>
			<div class='container-a'>
				<div class='c-title' v-on:click="show2 = !show2">
					<span style='color:#1476FE'>代理上级佣金设置</span>
					<span style='color:#FE2E5C;font-size: 16px;'>{{rate2}}</span>
				</div>
				<transition name="fade">
					<div v-if="show2">
						<div class='remind'>
							城市代理直推的首笔借款金额比例，请填写纯小数，比如0.75个点就填写0.0075，不要填0.75%。
						</div>
						<div class='contentx'>
						...
						<div class='remind' v-if="rateb!=='代理直推上级分佣比例'"> 您填写了：<span style='color:#FF654D'>{{rateb}}</span> , 如果城市代理直推放款10万。他上级能获得的佣金是：<span style='color:#FF654D'>{{rateB}}</span>  元。</div>
					</div>
				</transition>
			</div>
		</div>
		<div class='container1'>
			<div class='container-a'>
				...
				<transition name="fade">
					<div v-if="show3">
						<div class='remind'>
							城市代理直推的首笔借款金额比例，请填写纯小数，比如0.225个点就填写0.00225，不要填0.225%。
						</div>
						<div class='contentx'>
						...
						<div class='remind' v-if="ratec!=='代理直推上上级分佣比例'"> 您填写了：<span style='color:#FF654D'>{{ratec}}</span> , 如果城市代理直推放款10万。他上上级能获得的佣金是：<span style='color:#FF654D'>{{rateC}}</span>  元。</div>
					</div>
				</transition>
			</div>
		</div>
	...
	export default {
		...
		methods:{
			getData(){
				...
				this.axios.post('/api/toGetAgentExtension',{productsId:productsId})
				.then(function(res){
					...
					that.rate1 = res.data.agentExtension;
					that.rate2 = res.data.agentExtensionSuper;
					that.rate3 = res.data.agentExtensionBigSuper;
					that.$loading.hide();
			...
			toSaveExtension(){
				...
				this.axios.post('/api/toSaveExtension',{agentID:agentID,productsId:productsId,extension:extension})
				.then(function(ret){
			...
			toSaveSuperExtension(){
				...
				this.axios.post('/api/toSaveSuperExtension',{agentID:agentID,productsId:productsId,extension:extension})
				.then(function(ret){
					if(ret.data.code===500){
						that.$message.info('系统出错了');
						return;
			...
			toSaveBigSuperExtension(){
				...
				this.axios.post('/api/toSaveBigSuperExtension',{agentID:agentID,productsId:productsId,extension:extension})
				.then(function(ret){
					if(ret.data.code===500){
						that.$message.info('系统出错了');
						return;
